<x-layouts.app title="Comprar numeros - Sorteos CR" section="Sitio publico">
    ...
    <section class="mx-auto max-w-7xl space-y-5 pb-24 sm:space-y-6 xl:pb-8">
        <header class="relative overflow-hidden rounded-[1.5rem] bg-[#06110f] text-white shadow-2xl shadow-emerald-950/25 sm:rounded-[2rem]">
            ...
            <div class="relative grid min-h-[420px] content-end gap-5 p-5 sm:min-h-[500px] sm:p-7 lg:grid-cols-[1fr_340px] lg:items-end lg:p-8">
                <div class="max-w-3xl">
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="rounded-full bg-emerald-400 px-3 py-1.5 text-xs font-black uppercase tracking-wide text-emerald-950">{{ $raffle->sale_enabled ? 'Venta activa' : 'Venta pausada' }}</span>
                    ...
                    <p class="mt-5 text-xs font-black uppercase tracking-[0.24em] text-emerald-200">Sorteos CR</p>
                    ...
                    <p class="mt-2 text-base font-black text-emerald-200">{{ $raffle->draw_date ? 'Sorteo: '.$raffle->draw_date->format('d/m/Y') : 'Fecha por definir' }}</p>
                </div>
                ...
                <div class="rounded-2xl border border-white/15 bg-white/12 p-4 backdrop-blur-md sm:p-5">
                    <div class="flex items-end justify-between gap-4">
                        <div>
                            ...
                            <strong class="mt-1 block text-3xl font-black text-emerald-300">₡{{ number_format($raffle->price_per_package, 0, ',', ' ') }}</strong>
                    ...
                    <div class="mt-4 h-3 overflow-hidden rounded-full bg-white/18">
                        <div class="h-full rounded-full bg-gradient-to-r from-emerald-300 to-lime-300" style="width: {{ $soldPercent }}%"></div>
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-start">
            @if ($raffle->sale_enabled)
                <form class="order-1 overflow-hidden rounded-[1.35rem] border border-emerald-200 bg-white shadow-2xl shadow-emerald-950/10 xl:sticky xl:top-4 xl:order-2" method="post" action="{{ route('purchases.store', $raffle) }}" enctype="multipart/form-data" data-raffle-purchase data-random-url="{{ route('purchases.random', $raffle) }}" data-mode="{{ $raffle->assignment_mode }}" data-max-random-changes="{{ $raffle->max_random_changes }}" data-numbers-url="{{ route('raffles.numbers', $raffle) }}">
                    @csrf
                    <div class="bg-gradient-to-br from-emerald-700 to-[#073f35] p-5 text-white">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <p class="text-xs font-black uppercase tracking-[0.2em] text-emerald-200">Compra segura</p>
                                <h2 class="mt-1 text-3xl font-black tracking-tight">Tus numeros</h2>
                            </div>
                            <span class="rounded-full bg-white px-3 py-2 text-xs font-black text-emerald-800">Activa</span>
                        </div>
                        <p class="mt-3 text-sm font-semibold leading-6 text-emerald-50">Elige un paquete, cambia al azar si quieres y sube el comprobante para reservar.</p>
                    </div>
                    ...
                    <div class="grid gap-4 p-4 sm:p-5">
                        ...
                        <section class="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <span class="rounded-full bg-white px-2.5 py-1 text-xs font-black uppercase text-emerald-700">Hasta 5</span>
                            </div>
                            <div class="mt-3 grid grid-cols-2 gap-2" data-package-options>
                                @foreach ($packageOptions as $option)
                                    <button class="rounded-2xl border border-white bg-white px-3 py-4 text-center font-black leading-tight text-slate-950 shadow-sm transition hover:border-emerald-500 hover:bg-emerald-100" type="button" data-package="{{ $option['packages'] }}" data-quantity="{{ $option['quantity'] }}" data-amount="{{ $option['amount'] }}">
                        ...
                        @if ($raffle->assignment_mode === 'manual')
                            <section class="rounded-2xl border border-slate-200 bg-white p-4">
                                ...
                                <div class="flex items-center justify-between gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2">
                                    <button class="grid h-10 w-10 place-items-center rounded-xl bg-white text-xl font-black text-slate-800 shadow-sm transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-35" type="button" data-number-page-prev aria-label="Pagina anterior">‹</button>
                                    <strong class="text-sm font-black text-slate-600" data-number-page-label>Pagina 1</strong>
                                    <button class="grid h-10 w-10 place-items-center rounded-xl bg-white text-xl font-black text-slate-800 shadow-sm transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-35" type="button" data-number-page-next aria-label="Pagina siguiente">›</button>
                                </div>
                        ...
                        <section class="rounded-2xl border border-dashed border-emerald-300 bg-gradient-to-br from-emerald-50 to-white p-4">
                            <p class="text-sm font-black text-emerald-800">Seleccion actual</p>
                            ...
                            <p class="mt-3 text-2xl font-black text-emerald-700" data-total>Total: ₡0</p>
                        </section>
            ...
            <div class="order-2 min-w-0 space-y-5 xl:order-1">
                <section class="grid gap-3 sm:grid-cols-3">
                    <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <span class="text-xs font-black uppercase tracking-wide text-emerald-700">1. Escoge</span>
                    ...
                    <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <span class="text-xs font-black uppercase tracking-wide text-emerald-700">2. Paga</span>
                    ...
                    <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <span class="text-xs font-black uppercase tracking-wide text-emerald-700">3. Confirma</span>
                ...
                @if ($mediaItems->isNotEmpty())
                    <section class="rounded-[1.35rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-950/5 sm:p-6">
                        <div class="flex flex-wrap items-end justify-between gap-3">
                            <div>
                                <p class="text-xs font-black uppercase tracking-[0.22em] text-emerald-700">Galeria</p>
                                <h2 class="mt-1 text-2xl font-black tracking-tight">Fotos y videos del premio</h2>
                            </div>
                            <span class="rounded-full bg-emerald-50 px-3 py-2 text-xs font-black uppercase text-emerald-700">{{ $mediaItems->count() }} archivo(s)</span>
                        </div>
                ...
                @if ($raffle->public_sales_text)
                    <article class="overflow-hidden rounded-[1.35rem] border border-emerald-900/25 bg-[#06110f] text-white shadow-2xl shadow-emerald-950/10">
                        <div class="border-b border-white/10 bg-gradient-to-r from-emerald-950 to-[#06110f] p-5 sm:p-6">
                            <p class="text-xs font-black uppercase tracking-[0.22em] text-emerald-300">Evento relacionado</p>
                        ...
                        <div class="grid gap-3 border-t border-white/10 p-5 sm:grid-cols-3 sm:p-6">
                            ...
                            <div><p class="text-xs font-black uppercase tracking-wide text-slate-500">Precio</p><strong class="mt-1 block text-3xl font-black text-emerald-300">₡{{ number_format($raffle->price_per_package, 0, ',', ' ') }}</strong></div>
