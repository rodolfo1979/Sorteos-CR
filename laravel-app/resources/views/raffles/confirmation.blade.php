<x-layouts.app title="Confirmacion - Sorteos CR" section="Compra recibida">
    <section class="mx-auto max-w-4xl space-y-5 pb-20">
        <article class="overflow-hidden rounded-[1.5rem] border border-emerald-200 bg-white shadow-2xl shadow-emerald-950/10">
            <div class="bg-gradient-to-br from-emerald-700 to-[#073f35] p-6 text-white sm:p-8">
                <p class="text-xs font-black uppercase tracking-[0.22em] text-emerald-200">Orden {{ strtoupper(substr($order->public_uuid, 0, 8)) }}</p>
            ...
            <div class="grid gap-5 p-5 sm:p-6">
                <div class="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
                    <p class="text-sm font-black uppercase tracking-wide text-emerald-800">Tus tickets digitales</p>
                    <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                        @foreach ($order->numbers as $number)
                            <span class="relative flex min-h-14 items-center justify-center overflow-hidden rounded-xl bg-emerald-500 px-3 py-2 text-xl font-black tracking-wide text-white shadow-lg shadow-emerald-900/10 ring-1 ring-emerald-700/30">
