<x-layouts.app title="Comprar numeros - Sorteos CR" section="Sitio publico">
    ...
    <section class="mx-auto max-w-[1240px] pb-24 xl:pb-8">
        <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_430px] xl:items-start">
            <div class="min-w-0 space-y-5">
                <header class="relative overflow-hidden rounded-[1.4rem] bg-[#08080c] text-white shadow-2xl shadow-cyan-950/25 sm:rounded-[1.8rem]">
                    <div class="absolute inset-0">
                        @if ($heroImage)
                            <img class="h-full w-full object-cover opacity-60" src="{{ $heroImage }}" alt="Premio {{ $raffle->name }}">
                        @else
                        ...
                        @endif
                        <div class="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,.92),rgba(0,0,0,.58)_48%,rgba(0,0,0,.2)),linear-gradient(0deg,rgba(0,0,0,.78),transparent_58%)]"></div>
                    </div>

                    <div class="relative grid min-h-[360px] gap-5 p-5 sm:min-h-[430px] sm:p-7 lg:grid-cols-[1fr_280px] lg:items-end lg:p-8">
                        <div class="max-w-3xl self-end">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="rounded-full bg-amber-300 px-3 py-1.5 text-xs font-black uppercase tracking-wide text-[#111827]">{{ $raffle->sale_enabled ? 'Venta activa' : 'Venta pausada' }}</span>
                        ...
                        <div class="self-end rounded-2xl border border-white/15 bg-white/12 p-4 backdrop-blur-md sm:p-5">
                ...
                <section class="grid gap-3 sm:grid-cols-3">
                    <article class="rounded-2xl border border-white/10 bg-white/95 p-4 shadow-xl shadow-cyan-950/5">
                ...
                <article class="overflow-hidden rounded-[1.35rem] border border-white/10 bg-white shadow-xl shadow-cyan-950/8">
                    <div class="grid gap-5 p-5 sm:p-6 lg:grid-cols-[1fr_230px]">
                        <div>
                            <p class="text-xs font-black uppercase tracking-wide text-cyan-700">Premio</p>
                ...
                @if ($mediaItems->isNotEmpty())
                    <section class="rounded-[1.35rem] border border-white/10 bg-white p-5 shadow-xl shadow-cyan-950/8 sm:p-6">
                ...
                <article class="rounded-[1.35rem] border border-white/10 bg-white p-5 shadow-xl shadow-cyan-950/8 sm:p-6">
            ...
            <div class="xl:sticky xl:top-4">
                        ...
                        <div class="grid gap-4 p-4 sm:p-5">
                            ...
                            @if ($raffle->assignment_mode === 'manual')
                                <section class="rounded-2xl border border-slate-200 bg-white p-4">
                                    ...
                                    <div class="mt-3 grid max-h-[15rem] grid-cols-4 gap-1.5 overflow-auto pr-1 min-[420px]:grid-cols-5 sm:grid-cols-5 xl:grid-cols-5" data-number-grid>
                            ...
                            <section class="rounded-2xl border border-dashed border-amber-300 bg-gradient-to-br from-amber-50 to-white p-4">
                                <p class="text-sm font-black text-[#111827]">Seleccion actual</p>
                ...
                @else
                    <aside class="rounded-[1.35rem] border border-amber-200 bg-amber-50 p-5 shadow-xl shadow-amber-950/5">
